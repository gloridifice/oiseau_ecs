...
template <typename... T> class Query {
  ...
  public:
    Query(World* world1, std::unordered_set<Entity> entities)
        : world(world1), entities(std::move(entities)) {}
    ...
    class Iterator {
      ...
      public:
        Iterator(World* world, std::unordered_set<Entity>::iterator iter)
